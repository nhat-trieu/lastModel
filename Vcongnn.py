...
# ==========================================
CONFIG = {
    'face_dir':   '/kaggle/input/faces-features32face',
    ...
    'lr': 3e-5,                  # Reduced from 1e-4 to 5e-5
    'weight_decay': 1e-2,        # Reduced from 1e-4 to 5e-5
    'grad_clip': 0.5,            # Reduced from 1.0 to 0.5
    'epochs': 100,
    'patience': 15,              # Increased from 12 to 15
...
class VirtualNode_ConGNN_ECL(nn.Module):
    def __init__(self):
        ...
        self.clf_whole = nn.Linear(whole_dim, CONFIG['num_classes'])

    def forward(self, data):
        H_face = self.face_gat(data.face_x, data.face_edge_index)
        ...
        real_nodes_128 = torch.cat([H_face, H_context], dim=0)
        real_nodes_512 = self.proj_to_whole(real_nodes_128)
        
        real_batch = torch.cat([data.face_batch, data.context_batch], dim=0)
        
        updated_real = self.ipr_mpnn(real_nodes_512, batch=real_batch, training=self.training)
        
        whole_pooled = global_mean_pool(updated_real, real_batch)
        out_whole = self.clf_whole(whole_pooled)
...
def compute_ecl_loss_with_bpf(out_f, out_c, out_w, label, criterion):
    """
    ✅ FIXED: Implement ECL with BPF exactly as in ConGNN paper (Eq. 11, 12)
    
    Paper formula:
    BPF = (1 + λ * f(y_f, y_c)) * (L_f + L_c + L_w)
    where f(y_f, y_c) = 1 if y_f != y_c, else 0
    """
    # Get predictions
    y_f = out_f.argmax(dim=1)
    y_c = out_c.argmax(dim=1)
    
    # ✅ Per-sample indicator function f(y_f, y_c)
    # f = 1 when face and context disagree (emotion bias exists)
    # f = 0 when face and context agree (no emotion bias)
    bias_indicator = (y_f != y_c).float()  # Shape: [batch_size]
    
    # ✅ Per-sample penalty multiplier: 1 + λ * f(y_f, y_c)
    penalty_multiplier = 1.0 + CONFIG['lambda_penalty'] * bias_indicator  # Shape: [batch_size]
    
    # Compute losses for each branch
    # Use reduction='none' to get per-sample losses
    L_f_per_sample = F.cross_entropy(out_f, label, reduction='none')
    L_c_per_sample = F.cross_entropy(out_c, label, reduction='none')
    L_w_per_sample = F.cross_entropy(out_w, label, reduction='none')
    
    # Apply label smoothing if needed
    if CONFIG['label_smoothing'] > 0:
        n_classes = out_f.size(-1)
        with torch.no_grad():
            true_dist = torch.zeros_like(out_f)
            true_dist.fill_(CONFIG['label_smoothing'] / (n_classes - 1))
            true_dist.scatter_(1, label.unsqueeze(1), 1.0 - CONFIG['label_smoothing'])
        
        L_f_per_sample = torch.sum(-true_dist * F.log_softmax(out_f, dim=-1), dim=-1)
        L_c_per_sample = torch.sum(-true_dist * F.log_softmax(out_c, dim=-1), dim=-1)
        L_w_per_sample = torch.sum(-true_dist * F.log_softmax(out_w, dim=-1), dim=-1)
    
    # ✅ Apply BPF: (1 + λ * f) * (L_f + L_c + L_w) per sample
    total_loss_per_sample = penalty_multiplier * (L_f_per_sample + L_c_per_sample + L_w_per_sample)
    
    # Average over batch
    total_loss = total_loss_per_sample.mean()
    
    # For logging: calculate bias ratio (percentage of samples with bias)
    penalty_ratio = bias_indicator.mean().item()
    
    return total_loss, L_f_per_sample.mean().item(), L_c_per_sample.mean().item(), L_w_per_sample.mean().item(), penalty_ratio
