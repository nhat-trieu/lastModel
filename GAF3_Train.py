...
# ==========================================
CONFIG = {
    'face_dir':   '/kaggle/input/datasets/nguynnhtlam12/gaf-fearture/face_features_gaf3',
    'scene_dir':  '/kaggle/input/datasets/nguynnhtlam12/gaf-fearture/70accscene_features_gaf3000_v2/scene_features_final/scenes',
    'object_dir': '/kaggle/input/datasets/nguynnhtlam12/gaf-fearture/nosplitgaf3_object_features',
...
class ConGNN_Dataset(TorchDataset):
    def __init__(self, split='train', max_faces=32, max_objects=10, use_cache=None):
        # Map tên split cho từng modality
        face_split  = split                                           # train / val
        scene_split = 'Train' if split == 'train' else 'Validation'  # Train / Validation
        obj_split   = 'Train' if split == 'train' else 'Validation'  # Train / Validation
...
class SceneGuided_ConGNN(nn.Module):
    def __init__(self):
        ...
        self.reduce_face=nn.Sequential(
            nn.LayerNorm(CONFIG['face_dim']),nn.Dropout(0.2),nn.Linear(CONFIG['face_dim'],1024),
            nn.LayerNorm(1024),nn.ReLU(),nn.Dropout(drp),nn.Linear(1024,D),nn.LayerNorm(D),nn.ReLU())
        self.reduce_obj=nn.Sequential(
            nn.LayerNorm(CONFIG['object_dim']),nn.Dropout(0.2),nn.Linear(CONFIG['object_dim'],D),
            nn.LayerNorm(D),nn.ReLU(),nn.Dropout(drp))
        self.reduce_scene=nn.Sequential(
            nn.LayerNorm(CONFIG['scene_dim']),nn.Dropout(0.2),nn.Linear(CONFIG['scene_dim'],D),
            nn.LayerNorm(D),nn.ReLU(),nn.Dropout(drp))
...
class EarlyStopping:
    def __init__(self,patience=40,min_delta=0.001,path='best.pth'):
        ...
        self.counter=0; self.best_loss=np.inf; self.best_acc=0.0; self.early_stop=False

    def __call__(self,val_loss,val_acc,model):
        acc_improved = val_acc > (self.best_acc + self.min_delta)
        loss_improved = (val_loss < self.best_loss - self.min_delta) and (val_acc >= self.best_acc - 0.002)
        
        if acc_improved or loss_improved:
            torch.save(model.state_dict(), self.path)
            self.counter = 0
            # CHỈ CẬP NHẬT KHI THỰC SỰ LÀ ĐỈNH MỚI:
            if val_acc > self.best_acc: self.best_acc = val_acc
            if val_loss < self.best_loss: self.best_loss = val_loss
            print(f"  🔥 Best Saved! Loss={val_loss:.4f} | Acc={val_acc:.4f} (Peak: {self.best_acc:.4f})")
        else:
            self.counter += 1
